...
def take_events_name(event_league: str, sport: str) -> list:
    ...
    cursor.execute('''SELECT event_name, event_id, event_time FROM data WHERE event_league == ?''', [event_league])
...
def take_odds(id: str, sport: str) -> list:
    ...
    cursor = criar_(f'app/models/bd/odds/{sport}_odds_{verify_time()}.db')[0]
